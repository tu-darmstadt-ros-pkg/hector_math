...
/*!
 * Iterates over all indexes that lie in the given polygon and for each index (x, y) calls the given functor.
 * This method will iterate all cells where the center of the cell (x+0.5, y+0.5) is inside the polygon.
 * Note: The polygon has to be in the index space, hence, if it is in map coordinates it might be necessary to divide
...
void iteratePolygon( const Polygon<Scalar> &polygon,
...
template<typename Scalar, typename Functor>
void iteratePolygon( const Polygon<Scalar> &polygon, Eigen::Index rows, Eigen::Index cols, Functor functor )
{
...
void iteratePolygon( const Polygon<Scalar> &polygon,
                     ...
                     Functor functor )
{
  if ( polygon.cols() < 3 ) return;
  if ( polygon.cols() <= 30 )
  {
    detail::iteratePolygon<Scalar, Functor, 30>( polygon, row_min, row_max, col_min, col_max, functor );
  }
  else
  {
...
void iteratePolygon( const Polygon<Scalar> &polygon,
                     ...
                     Functor functor )
{
  // Build iteration lines from the polygon points that allow us to get the x value for each discrete y index in the map
  struct Line
  {
    Line() = default;
    ...
    Line( const Point<Scalar> &a, const Point<Scalar> &b )
    {
      ...
      if ( a.y() > b.y()) std::swap( start, end );
      Scalar diff_y = b.y() - a.y();
      if ( std::abs( diff_y ) < 1E-4 ) x_increment = 0;
      else x_increment = (b.x() - a.x()) / (b.y() - a.y());
  ...
  using LineContainer = typename std::conditional<LIMIT == 0, std::vector<Line>, BoundedVector<Line, LIMIT + 1>>::type;
  using LinePointerContainer = typename std::conditional<
    LIMIT == 0, std::vector<Line *>, BoundedVector<Line *, LIMIT>>::type;
  using RegionContainer = typename std::conditional<
    LIMIT == 0, std::vector<Scalar>, BoundedVector<Scalar, LIMIT>>::type;
  ...
  std::sort( lines.begin(), lines.end(), []( const Line &a, const Line &b ) { return a.start.y() < b.start.y(); } );
  size_t active_line_index = 0;
  LinePointerContainer active_lines;
  RegionContainer x_region_segments;

  Eigen::Index y = std::max<Eigen::Index>( col_min, std::round( lines[active_line_index].start.y()));
  for ( ; y < max_y; ++y )
  {
    for ( int i = active_lines.size() - 1; i >= 0; --i )
    {
      active_lines[i]->x += active_lines[i]->x_increment;
      if ( active_lines[i]->end.y() >= y + 0.5 ) continue;
      active_lines.erase( active_lines.begin() + i );
    }
    for ( size_t i = active_line_index; i < lines.size(); ++i )
    {
      if ( lines[i].start.y() >= y + 0.5 ) break;
      if ( lines[i].start.y() < y ) lines[i].x += lines[i].x_increment;
      active_lines.push_back( &lines[i] );
      ++active_line_index;
    }
    // We obtain from each line the x for the current y and use that information to iterate between each pair of
    // x(k) -> x(k+1) where k = 2 * i and i is a natural integer
    ...
    if ( x_region_segments.size() > 2 )
    {
      ...
      {
        if ( x_region_segments[i - 1] != x_region_segments[i] ) continue;
    ...
    for ( size_t i = 0; i < x_region_segments.size() - 1; i += 2 )
    {
      ...
      for ( Eigen::Index x = x_start; x < x_end; ++x )
      {
        functor( x, y );
      }
    }
