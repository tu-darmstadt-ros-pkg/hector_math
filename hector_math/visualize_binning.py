...
def inverse_bin(val):
    if (val & 0b11) == 0b01:
        ...
        z = (
            (((val >> (3 + DIM_BITS)) & (2**DIM_BITS - 1)) - 0.5) / AXIS_BINS_2 - 1
        ) / np.sqrt(2)
        x = np.sqrt(1 - y**2 - z**2)
        if (val & 0b100) == 0b100:
            x = -x
...
def inverse_sf(x, y, z):
    ...
    k = max(
        2.0,
        np.floor(
            np.log(fibonacci_n * np.pi * np.sqrt(5) * (1.0 - cos_theta**2))
            / np.log(PHI + 1)
        ),
    )
    ...
    b = np.array(
        [
            [
                2 * np.pi * madfrac(f0 + 1, PHI - 1) - 2 * np.pi * (PHI - 1),
    ...
    d, j = np.inf, 0
    for s in range(4):
        cos_theta = np.dot(b[1, :], np.array([s % 2, s // 2]) + c) + (
            1 - 1 / fibonacci_n
        )
...
if __name__ == "__main__":
    ...
    vectors = (
        np.array([inv_bin_fn(x) for x in bins]) if not SHOW_INPUT else input_vectors
    )
