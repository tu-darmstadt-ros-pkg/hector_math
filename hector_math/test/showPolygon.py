...
def draw_nice_visualisation(
    corners, real_points, iterated_points, limits, name="Visualization"
):
    plt.figure()
    ...
    # draw centers
    centers = np.array(
        np.meshgrid(np.arange(minx, maxx) + 0.5, np.arange(miny, maxy) + 0.5)
    ).reshape(2, (maxx - minx) * (maxy - miny))
    plt.scatter(centers[0, :], centers[1, :])
    # draw iterated points (found during testing)
    plt.scatter(
        [x[0] + 0.5 for x in iterated_points],
        [x[1] + 0.5 for x in iterated_points],
        s=100,
        facecolors="none",
        edgecolors="red",
        linewidths=2,
        label="iterated Positions",
    )
    # draw 'groundtruth' data
    plt.scatter(
        [x[0] + 0.5 for x in real_points],
        [x[1] + 0.5 for x in real_points],
        s=120,
        marker="x",
        c="green",
        linewidths=2,
    ...
    # draw limits
    plt.plot(
        [limits[0], limits[0], limits[1] - 0.25, limits[1] - 0.25, limits[0]],
...
def show_all_failure_cases():
    ...
    for file_name in all_files:
        ...
        modification_time = time.strftime(
            "%d/%m/%Y %H:%M", time.localtime(os.path.getmtime(file))
        )
        iterated_points, real_points, corners, limits = read_from_file(file)
        draw_nice_visualisation(
            corners,
            real_points,
            iterated_points,
            limits,
