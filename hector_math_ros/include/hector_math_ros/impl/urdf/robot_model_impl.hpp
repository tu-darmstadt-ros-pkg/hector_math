...
{
  return joint_info.getTransform( this->joint_positions_ );
}

template<typename Scalar>
Scalar UrdfRobotModel<Scalar>::computeWeightedCenterOfMass( const LinkTree &root,
...
UrdfRobotModel<Scalar>::computeAxisAlignedBoundingBox( const LinkTree &root,
                                                       const Isometry3<Scalar> &transform ) const
{
...
UrdfRobotModel<Scalar>::buildLinkTree( const urdf::LinkSharedPtr &root, Scalar &mass )
{
  ...
  auto collision_array = root->collision_array.empty()
                             ? std::vector<urdf::CollisionSharedPtr>( 1, root->collision )
  ...
  if ( result.geometries.empty() && settings_.fallback_on_visual ) {
    auto visual_array = root->visual_array.empty()
                            ? std::vector<urdf::VisualSharedPtr>( 1, root->visual )
                            : root->visual_array;
  ...
  for ( const urdf::JointSharedPtr &joint : root->child_joints ) {
    for ( const urdf::LinkSharedPtr &link : root->child_links ) {
      ...
      child.parent_to_joint_transform =
